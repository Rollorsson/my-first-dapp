...
export function SwapInterface() {
  ...
  async function fetchBalances() {
    ...
    try {
      ...
      const btkBalanceResult = await publicClient.readContract({
        address: BUENA_TOKEN as `0x${string}`,
        abi: ERC20_ABI,
        functionName: "balanceOf",
        args: [address],
      });
  ...
  async function handleGetQuote() {
    ...
    try {
      ...
      const zeroForOne = (CELO_TOKEN as string).toLowerCase() < (BUENA_TOKEN as string).toLowerCase();

      const poolKey = {
        currency0: zeroForOne ? (CELO_TOKEN as `0x${string}`) : (BUENA_TOKEN as `0x${string}`),
        currency1: zeroForOne ? (BUENA_TOKEN as `0x${string}`) : (CELO_TOKEN as `0x${string}`),
        fee: FEE_TIER,
    ...
    try {
      ...
      if (erc20Allowance < amountIn) {
        ...
        const approveTx = await walletClient.writeContract({
          ...
          args: [PERMIT2_ADDRESS as `0x${string}`, BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")],
        });
      ...
      const permit2Allowance = await publicClient.readContract({
        ...
        abi: PERMIT2_ABI,
        functionName: "allowance",
        args: [address, CELO_TOKEN as `0x${string}`, UNIVERSAL_ROUTER as `0x${string}`],
      });
      ...
      if (needsPermit2Approval) {
        ...
        const permit2ApproveTx = await walletClient.writeContract({
          ...
          abi: PERMIT2_ABI,
          functionName: "approve",
          args: [CELO_TOKEN as `0x${string}`, UNIVERSAL_ROUTER as `0x${string}`, maxUint160, deadline],
        });
